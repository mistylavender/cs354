...
void replay_trace(char* trace_fn) {
    ...
    while (fgets(buf, 1000, trace_fp) != NULL) {
        // Skip lines not starting with a space (e.g., empty lines)
        if (buf[0] != ' ') continue;

        // Find the operation (skip leading spaces after the initial space)
        int op_pos = 1;
        while (buf[op_pos] == ' ' && op_pos < strlen(buf)) op_pos++;

        char op = buf[op_pos];
        if (op != 'L' && op != 'S' && op != 'M') continue;

        // Extract address and length (skip spaces after the operation)
        char* addr_start = buf + op_pos + 1;
        while (*addr_start == ' ') addr_start++;
        sscanf(addr_start, "%llx,%u", &addr, &len);
